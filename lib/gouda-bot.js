...
const errorMessages = [{
		message: {
			attachment: {
				type: "image",
				payload: {
					url: "https://media.giphy.com/media/oQ6P5nknXM40w/giphy.gif"
				}
			}
		}
	}, {
                message: {
                        attachment: {
                                type: "image",
                                payload: {
                                        url: "https://media.giphy.com/media/gpufDFw0sPBYY/giphy.gif"
                                }
                        }
                }
        }, {
                message: {
                        attachment: {
                                type: "image",
                                payload: {
                                        url: "https://media.giphy.com/media/11NBUrJDuMd5As/giphy.gif"
                                }
                        }
                }
        }, {
                message: {
                        attachment: {
                                type: "image",
                                payload: {
                                        url: "https://media.giphy.com/media/JwVWLRnZkh2M0/giphy.gif"
                                }
                        }
                }
        }, {
                message: {
                        attachment: {
                                type: "image",
                                payload: {
                                        url: "https://media.giphy.com/media/9ohlKnRDAmotG/giphy.gif"
                                }
                        }
                }
        }
...
exports.receivedMessage = (messagingEvent, userStates) => {
	...
	if (messagingEvent.message.hasOwnProperty('quick_reply') && messagingEvent.message.quick_reply.payload === "RIGHT_FORMAT") {
		...
		if (messagingEvent.message.text.indexOf('cena') > -1) {
			sendResponse({
				recipient: {id: recipientId},
                		message: {
                        		attachment: {
                                		type: "image",
                                		payload: {
                                        		url: "https://media.giphy.com/media/12XNRtl6kZxQVq/giphy.gif"
                                		}
                        		}
                		}
			}, {
			...
			sendResponse({
				recipient: {id: recipientId},
				message: errorMessages[Math.floor(Math.random() * errorMessages.length)].message
			}, {
                                recipient: {id: recipientId},
                                message: {
                                        text: "The text you entered is incorrect - please ensure you follow the right format"
